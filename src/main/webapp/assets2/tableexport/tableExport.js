    ...
    tableExport: function (options) {
      ...
      if (defaults.type == 'csv' || defaults.type == 'tsv' || defaults.type == 'txt') {
        ...
        $tables.each(function(){
          ...
          len -= defaults.unExportCol.length;//减去不导出列个数
          var title = '<table><tr><td align=center colspan='+len+'><font size="5" ><b>'+defaults.worksheetName+
          '</b></font></td></tr></table><table  border="1"  rull="all" style="border-collapse: collapse"><thead>';
          docData += title;
          if(defaults.firstRow.length>0){
        	  docData +="<tr><td colspan="+len+">"+defaults.firstRow+"</td></tr>";
          }
          // Header
          ...
          $hrows.each(function() {
            ...
            ForEachVisibleCell(this, 'th,td', rowIndex, $hrows.length,
              function(cell, row, col) {
                if (cell !== null) {
                	//跳过不导出列
                	if(0<=defaults.unExportCol.indexOf(col)){
                   	 return;
                     }
                  var thstyle = '';
                  trData += "<td border='1'  rull='all' style='border-collapse: collapse;'";
      ...
      function downloadFile(filename, header, data) {
        ...
        if (filename !== false && (ua.indexOf("MSIE ") > 0 || ua.match(/Edge/) || !!ua.match(/Trident.*rv\:11\./))) {
